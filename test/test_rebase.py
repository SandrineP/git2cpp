...
def test_rebase_basic(repo_init_with_commit, commit_env_config, git2cpp_path, tmp_path):
    ...
    file_path = tmp_path / "feature_file.txt"
    ...
    file_path_2 = tmp_path / "master_file.txt"
    ...
    p_checkout_feature = subprocess.run(
        checkout_feature_cmd, capture_output=True, cwd=tmp_path, text=True
    )
    ...
    assert (tmp_path / "feature_file.txt").exists()
...
def test_rebase_multiple_commits(repo_init_with_commit, commit_env_config, git2cpp_path, tmp_path):
    ...
    subprocess.run([git2cpp_path, "add", "master_file.txt"], cwd=tmp_path)
    ...
    assert (tmp_path / "file_3.txt").exists()
    assert (tmp_path / "master_file.txt").exists()
...
def test_rebase_abort(repo_init_with_commit, commit_env_config, git2cpp_path, tmp_path):
    ...
    p_abort = subprocess.run(abort_cmd, capture_output=True, cwd=tmp_path, text=True)
...
def test_rebase_skip(repo_init_with_commit, commit_env_config, git2cpp_path, tmp_path):
    ...
    p_skip = subprocess.run(skip_cmd, capture_output=True, cwd=tmp_path, text=True)
    assert p_skip.returncode == 0
    assert "Skipping" in p_skip.stdout


def test_rebase_quit(repo_init_with_commit, commit_env_config, git2cpp_path, tmp_path):
    ...
    p_quit = subprocess.run(quit_cmd, capture_output=True, cwd=tmp_path, text=True)
...
def test_rebase_onto(repo_init_with_commit, commit_env_config, git2cpp_path, tmp_path):
    ...
    subprocess.run([git2cpp_path, "add", "file1.txt"], cwd=tmp_path)
    subprocess.run([git2cpp_path, "commit", "-m", "branch1 commit"], cwd=tmp_path)

    # Create second branch from branch1
    subprocess.run([git2cpp_path, "checkout", "-b", "branch2"], cwd=tmp_path)
    file2 = tmp_path / "file2.txt"
    file2.write_text("branch2")
    subprocess.run([git2cpp_path, "add", "file2.txt"], cwd=tmp_path)
    ...
    subprocess.run([git2cpp_path, "add", "target.txt"], cwd=tmp_path)
    subprocess.run([git2cpp_path, "commit", "-m", "target commit"], cwd=tmp_path)
...
def test_rebase_invalid_upstream_error(
    ...
    assert p_rebase.returncode != 0
    assert (
        "could not resolve upstream" in p_rebase.stderr
        or "could not resolve upstream" in p_rebase.stdout
    )
...
def test_rebase_already_in_progress_error(
    ...
    assert p_rebase.returncode != 0
    assert (
        "rebase is already in progress" in p_rebase.stderr
        or "rebase is already in progress" in p_rebase.stdout
    )
...
def test_rebase_continue_with_unresolved_conflicts(
    repo_init_with_commit, commit_env_config, git2cpp_path, tmp_path
):
